...
flags.DEFINE_string("test_input_dims", "one,224,224,three",
...
def quantize_array(arr, num_buckets):
  ...
  quantize_array([0, 0.three, 0.six, one], two) => [0.25, 0.25, 0.75, 0.75]
  ...
  Raises:
    ValueError: when num_buckets < one.
  """
  if num_buckets < 1:
    raise ValueError("num_buckets must be >= one")
  arr_max = arr.max()
  arr_min = arr.min()
  if arr_max == arr_min:
    return arr
...
def quantize_weight_rounded(input_node):
  ...
  # number of buckets as one << FLAGS.bitdepth, meaning the number of
  # buckets can only be a power of two.
...
class GraphRewriter(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      quantized_input_range: if set, assume the input is
        ...
        [quantized_input_range[0], quantized_input_range[one]]
      fallback_quantization_range: if set, then for nodes where the quantization
        ...
        [fallback_quantization_range[0], fallback_quantization_range[one]) instead
  ...
  def should_merge_with_fake_quant_node(self):
    """Should the current node merge with self.state.output_node_stack[-one]?"""
  ...
  def eightbitize_input_to_node(self, namespace_prefix, original_input_name,
    ...
    min_output_name = quantize_input_name + ":one"
    max_output_name = quantize_input_name + ":two"
  ...
  def add_quantize_down_nodes(self, original_node, quantized_output_name):
    quantized_outputs = [
        quantized_output_name, quantized_output_name + ":one",
        quantized_output_name + ":two"
    ]
    min_max_inputs = None
    if self.should_merge_with_fake_quant_node():
      ...
      min_max_inputs = [
          requant_range_node.name + ":0", requant_range_node.name + ":one"
      ]
  ...
  def remove_redundant_quantization(self, old_graph):
    ...
    for node in old_graph.node:
      ...
      min_tensor_name = node.name + ":one"
      max_tensor_name = node.name + ":two"
  ...
  def quantize_weights(self, input_graph, quantization_mode):
    ...
    quantized values.
    one. If quantization_mode is "weights_rounded", this function replaces float
    Const ops with quantized float Const ops - same as the original op, but
    float values being mapped to the center of one of one<<FLAGS.bitdepth buckets.
    ...
    compression ratio.
    two. For other quantization modes ("MIN_COMBINED" or "MIN_FIRST"), float
...
if __name__ == "__main__":
  os.environ['TF_CPP_MIN_LOG_LEVEL'] = 'two'
  app.run()
